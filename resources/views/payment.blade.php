                    ...
                    @else
                        ...
                        <div class="pr-10">
                            <div class="p-4 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100">
                                <div>
                                    <p><span class="underline">From </span>: {{ $couriers->from }}</p>
                                </div>
                                <div>
                                    <p><span class="underline">To </span>: {{ $couriers->to }}</p>
                                </div>
                                <div>
                                    <p><span class="underline">Weight </span>: {{ $couriers->weight }} gm</p>
                                </div>
                                <div>
                                    <p><span class="underline">Distance </span>: {{ $couriers->distance }} KM</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2">
                            ...
                            <form action="/payment" method="POST">
                                ...
                                <input type="hidden" value="{{ $couriers->recipient_name }}" name="recipient_name"
                                    readonly>
                                <input type="hidden" value="{{ $couriers->weight }}" name="weight" readonly>
                                <input type="hidden" value="{{ $couriers->sender_number }}" name="sender_number"
                                    readonly>
                                <input type="hidden" value="{{ $couriers->recipient_number }}" name="recipient_number"
                                    readonly>
                                <input type="hidden" value="{{ $couriers->from }}" name="from" readonly>
                                <input type="hidden" value="{{ $couriers->to }}" name="to" readonly>
                                <input type="hidden" value="{{ $couriers->sender_address }}" name="sender_address"
                                    readonly>
                                <input type="hidden" value="{{ $couriers->recipient_address }}"
                                ...
                                <button type="submit"
                                    class="p-2 text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-blue-500 dark:text-blue-500 dark:hover:text-white dark:hover:bg-blue-500 dark:focus:ring-blue-800">Pay
                                    Now</button>
                            </form>
                        </div>
                    @endif
                        ...
                        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                            ...
                            <li>
                                <div class="flex items-center">
                                    ...
                                    <a href="/payment"
                                        class="ms-1 text-sm font-medium text-blue-500 hover:text-red-600 dark:hover:text-yellow-500 md:ms-2 dark:text-gray-400">Payment</a>
                                </div>
                            </li>
